function [all_feats]=getWindowedFeats(raw_data, fs, window_length, window_overlap, no_features)
    %
...
all_feats = zeros(num_windows, no_features*size(new_data,2));

% Process feature for first window
all_feats(1,:) = get_features(new_data(idx,:), fs);
...
for win = 2:num_windows
    ...
    idx = lower:1:upper;
    all_feats(win,:) = get_features(new_data(idx,:), fs);
    lower = lower + winDisp;
    upper = upper + winDisp;
end
end